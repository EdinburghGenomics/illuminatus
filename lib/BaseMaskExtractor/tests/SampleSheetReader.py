...
class SampleSheetReader:
	...
	def _get_column_mapping_from_ssfile( self , SampleSheetFile ):
        	...
        	csvData = csv.reader(open(csvFile,'rU'), delimiter=',')
		column_mapping = {}
		found_header = False
		samplesheet_data = []
        	try:
...
def main():
	...
	column_mapping = {} 
	# e.g.: {'index': 5, 'description': 9, 'sample_id': 0, 'sample_plate': 2, 'i7_index_id': 6, 'sample_well': 3, 'sample_project': 4, 'sample_name': 1, 'index2': 7, 'i5_index_id': 8}

	try:
		for row in csvData:
			if found_header:
				index_length = get_index_length(row)[0]  +  get_index_length(row)[1]
				#print index_length

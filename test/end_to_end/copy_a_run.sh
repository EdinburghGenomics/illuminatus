...
for lane in $LANES ; do
  cp -v "$RUN_PATH"/Data/Intensities/BaseCalls/$lane/s_*.filter "$DEST"/Data/Intensities/BaseCalls/$lane/
  #Only for MiSeq machines I think
  cp -v "$RUN_PATH"/Data/Intensities/BaseCalls/$lane/s_*.control "$DEST"/Data/Intensities/BaseCalls/$lane/ || true
  ...
  if [ -e "$RUN_PATH"/Data/Intensities/$lane ] ; then
    ...
    cp -v "$RUN_PATH"/Data/Intensities/$lane/s_*.*locs "$DEST"/Data/Intensities/$lane/
  fi

  CYCLES="`ls "$RUN_PATH"/Data/Intensities/BaseCalls/$lane | grep -x 'C[0-9]\+.1'`"
  echo "Copying `wc -w <<<$CYCLES` cyles of bcl[.gz] files for all tiles of $lane..."
  for cycle in $CYCLES ; do
    mkdir "$DEST"/Data/Intensities/BaseCalls/$lane/$cycle
    cp "$RUN_PATH"/Data/Intensities/BaseCalls/$lane/$cycle/s_*.bcl* "$DEST"/Data/Intensities/BaseCalls/$lane/$cycle/
    #We may or may not have these files
    cp "$RUN_PATH"/Data/Intensities/BaseCalls/$lane/$cycle/s_*.stats "$DEST"/Data/Intensities/BaseCalls/$lane/$cycle/ 2>/dev/null || true
  done
done
...
if [ -L "$RUN_PATH/SampleSheet.csv" ] ; then
    ...
    echo "You may remove this if not needed."

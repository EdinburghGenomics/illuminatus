...
echo "==> Last few errors in $myrun/pipeline.log <==" | env GREP_COLORS='mt=01;35' grep --color=always '.*'

grep --color=always '^Error executing rule' "$myrun"/pipeline.log | tail -n 5
echo

lastfail=`grep '^Error executing rule' "$myrun"/pipeline.log | grep -o 'external: [0-9]\+' | awk '{print $2}' | tail -n 1`
